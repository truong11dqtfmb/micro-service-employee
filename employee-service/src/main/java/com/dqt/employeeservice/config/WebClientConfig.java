package com.dqt.employeeservice.config;
...
public class WebClientConfig {
    ...
    public PositionClient positionClient() {
        HttpServiceProxyFactory httpServiceProxyFactory =
                HttpServiceProxyFactory
                        .builder(WebClientAdapter.forClient(positionWebClient()))
                        .build();
        return httpServiceProxyFactory.createClient(PositionClient.class);
    }

    public WebClient positionWebClient() {
        return WebClient
                .builder()
                .baseUrl("http://position-service")
                .filter(filterFunction)
                .build();
    }
